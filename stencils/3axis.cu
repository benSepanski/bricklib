...
__global__ void
d3pt7complex_brick(unsigned (*grid)[STRIDEB][STRIDEB], ComplexBrick bIn, ComplexBrick bOut,
                   bComplexElem *coeff) {
  long tk = GB + blockIdx.z;
  long tj = GB + blockIdx.y;
  long ti = GB + blockIdx.x;
...
__global__ void
d3pt7complex_arr_warp(bComplexElem (*arr_in)[STRIDE][STRIDE], bComplexElem (*arr_out)[STRIDE][STRIDE], bComplexElem *coeff) {
  long tk = PADDING + GZ + blockIdx.z * TILE;
  long tj = PADDING + GZ + blockIdx.y * TILE;
  long i = PADDING + GZ + blockIdx.x * 32 + threadIdx.x;
  for (int k = tk; k < tk + TILE; ++k)
...
void d3pt7complexcu() {
  ...
  unsigned size = STRIDE * STRIDE * STRIDE * sizeof(bComplexElem);
  ...
  bComplexElem *coeff_dev;
  {
    unsigned size = 129 * sizeof(bComplexElem);
    cudaMalloc(&coeff_dev, size);
    cudaMemcpy(coeff_dev, zCoeff, size, cudaMemcpyHostToDevice);
  }
  ...
  auto brick_func = [&grid, &bInfo_dev, &bStorage_dev, &coeff_dev]() -> void {
    ...
    dim3 block(NB, NB, NB), thread(BDIM);
    d3pt7complex_brick << < block, thread >> > (grid, bIn, bOut, coeff_dev);
  };
  ...
  auto cuarr_func = [&in_dev, &out_dev, &coeff_dev]() -> void {
    ...
    dim3 block(NB, NB, NB), thread(BDIM);
    d3pt7complex_arr << < block, thread >> > (arr_in, arr_out, coeff_dev);
  };
