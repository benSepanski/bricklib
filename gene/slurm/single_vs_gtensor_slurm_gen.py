...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--output_file", type=str, help="Output file to write to", default="vs_gtensor_out.csv")
...
"""
    ...
    || exit 1
    (cd {build_dir} && make clean && make -j 20 single-gene-6d || exit 1)
"""
...
"""
    ...
    brick_dim = (2, 16, 2, 2, 1, 1)
    vec_dim = (2, 16, 1, 1, 1, 1)
    assert all([b % v == 0 for (b, v) in zip(brick_dim, vec_dim)])
    run_jobs = f"""
    {run_job(per_process_extent, True)}
    for i in {{1..99}} ; do
        echo "Job $i of 99"
        {run_job(per_process_extent, False)}
    done ;
"""
    ...
    slurm_script = "\n\n".join([preamble] + environment_setup + [build_job(brick_dim, vec_dim), run_jobs]) + "\n"
