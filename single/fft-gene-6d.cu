...
double complex_to_complex_1d_j_fft_brick_transpose(bComplexElem *in_arr, bComplexElem *out_arr, int warmup, int iter, int direction = CUFFT_FORWARD)
{
  ...
  plan.setup(bIntermed_dev, grid_ptr_transposed_dev, bIntermed_dev, grid_ptr_transposed_dev);
  auto compute_fft = [&plan, &bInfo, &bIn_dev, &bIntermed_dev, &bOut_dev](bool direction = FFTPlanType::BRICKS_FFT_FORWARD) -> void {
    // transpose into intermediate brick
    transpose_brick_ij<< <bInfo.nbricks, 64>> >(bIn_dev, bIntermed_dev);
    // // compute FFT on intermed-brick
    plan.launch(direction); 
    // transpose intermed back into tou-brick
    transpose_brick_ij<< <bInfo.nbricks, 64>> >(bIntermed_dev, bOut_dev);
  };
  // time function (and compute fft in process)
  double num_seconds = cutime_func(compute_fft, warmup, iter);
...
int main(int argc, char **argv)
{
  ...
  if(run_1d_j_brick)
  {
    ...
    std::cout << std::setw(colWidth) << "cufft_1d_j_brick_transpose"
              << std::setw(colWidth) << cufft_1d_j_brick_transpose_num_seconds
              << std::endl;
    nvtxRangePop();
